...
function cutInLine(arr, item) {
    var solution = arr.splice(1, 0, item);
    return arr;
}
...
function tripleTheFun(thing) {
  
    var num = Number(thing)
    if(Number.isNaN(num)) {
...
function spiderTest(actorObj, name) {
    for (var key in actorObj) {
        if(actorObj[key] === "Andrew Garfield") {
            actorObj.spiderRating = 10;
        }
        else if(actorObj[key] === "Tobey Maguire") {
            actorObj.spiderRating = 0;
        }
        else {
            actorObj.spiderRating = 'not a spiderman';
        }
    }
